...
describe('MCP repository policy', () => {
  ...
  it('preserves OpenClaw configuration aliases when canonical variables are absent', async () => {
    const backend = createBackend();
    const policy = await createMcpRepositoryPolicy(backend, {
      OPENCLAW_CODE_INDEX_ALLOWED_REPOS: 'Beta',
      OPENCLAW_CODE_INDEX_DEFAULT_REPO: 'Beta',
    });
    await policy.scopeBackend(backend).callTool('explore', { name: 'auth' });
    expect(backend.callTool).toHaveBeenCalledWith('explore', {
      name: 'auth',
      repo: '/repos/beta',
    });
  });
  ...
  ])('fails closed for explicitly blank repository configuration', async (env) => {
