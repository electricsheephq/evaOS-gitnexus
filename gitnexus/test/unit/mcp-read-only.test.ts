...
function enableReadOnly(): void {
  ...
  vi.stubEnv('OPENCLAW_CODE_INDEX_MCP', '0');
}
...
describe('MCP read-only mode', () => {
  ...
  it('preserves the OpenClaw activation alias', async () => {
    vi.stubEnv('GITNEXUS_MCP_READ_ONLY', '0');
    vi.stubEnv('OPENCLAW_CODE_INDEX_MCP', '1');
    const session = await connect();
    try {
      const response = await session.client.listTools();
      expect(response.tools.map((tool) => tool.name).sort()).toEqual(READ_ONLY_TOOLS);
  ...
  it.each([
    ...
    ['GITNEXUS_MCP_READ_ONLY', 'banana'],
    ['OPENCLAW_CODE_INDEX_MCP', 'yes'],
  ])('fails startup for malformed %s=%s', (name, value) => {
    vi.stubEnv('GITNEXUS_MCP_READ_ONLY', name === 'GITNEXUS_MCP_READ_ONLY' ? value : '0');
    vi.stubEnv('OPENCLAW_CODE_INDEX_MCP', name === 'OPENCLAW_CODE_INDEX_MCP' ? value : '0');
    expect(() => createMCPServer(createMockBackend() as unknown as LocalBackend)).toThrow(
      /must be 0 or 1/i,
    );
  });

  it('validates the compatibility alias even when canonical read-only mode is enabled', () => {
    vi.stubEnv('GITNEXUS_MCP_READ_ONLY', '1');
    vi.stubEnv('OPENCLAW_CODE_INDEX_MCP', 'malformed');
    expect(() => createMCPServer(createMockBackend() as unknown as LocalBackend)).toThrow(
      /OPENCLAW_CODE_INDEX_MCP must be 0 or 1/i,
    );
  });
});
