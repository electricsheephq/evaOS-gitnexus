...
| `GITNEXUS_LBUG_BUFFER_POOL_SIZE`             | integer `>= 0` (bytes)       | min(2 GiB, 80% RAM) | LadybugDB buffer-pool ceiling for every GitNexus database. Bounded so a long-lived MCP process cannot grow toward LadybugDB's native 80%-of-RAM default. `0` restores that native default; invalid values warn and fall back. |
| `GITNEXUS_LBUG_MAX_DB_SIZE`                  | positive integer (bytes)     | `17179869184` (16 GiB) | Upper bound for one LadybugDB file. This is an mmap/disk-address-space ceiling, not a memory limit. |
