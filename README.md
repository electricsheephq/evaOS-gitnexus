...
| `GITNEXUS_LBUG_BUFFER_POOL_SIZE`           | min(2 GiB, 80% RAM)       | LadybugDB buffer-pool ceiling in bytes for every GitNexus database. `0` restores LadybugDB's native 80%-of-RAM default; invalid values warn and fall back. | A long-lived MCP process or large analyze needs a different page-cache ceiling. |
| `GITNEXUS_LBUG_MAX_DB_SIZE`                | `17179869184` (16 GiB)    | Maximum size in bytes for one LadybugDB file. This is an mmap/disk-address-space ceiling, not a memory limit. | A genuinely huge repository approaches the default database-file ceiling. |
